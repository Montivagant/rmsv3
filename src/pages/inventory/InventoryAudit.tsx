...
export default function InventoryAudit({ newAudit = false }: InventoryAuditProps) {
  ...
  const handleViewAudit = useCallback((audit: InventoryCount) => {
    navigate(`/inventory/audit/${audit.id}`);
  }, [navigate]);

  const handleResumeAudit = useCallback((audit: InventoryCount) => {
    navigate(`/inventory/audit/${audit.id}/entry`);
  }, [navigate]);

  const handleExportAudit = useCallback((audit: InventoryCount) => {
    showToast({
