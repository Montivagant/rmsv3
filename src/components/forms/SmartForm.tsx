...
export interface FormField {
  ...
  type: 'text' | 'email' | 'tel' | 'number' | 'currency' | 'sku' | 'textarea' | 'select' | 'checkbox-group';
  ...
  optionGroups?: FormFieldOptionGroup[] | undefined;
...
type ValidatedInputType = 'text' | 'email' | 'tel' | 'number' | 'currency' | 'sku';

const sanitizeFormValue = (value: string): string => value.replace(/[<>`]/g, '');

function isValidatedInputType(type: FormField['type']): type is ValidatedInputType {
  return type === 'text' ||
    type === 'email' ||
    type === 'tel' ||
    type === 'number' ||
...
export function SmartForm({
  ...
  const handleFieldChange = (fieldName: string, value: any) => {
    ...
    } else if (field?.formatter && valueFormatters[field.formatter] && typeof value === 'string') {
  ...
  const renderField = (field: FormField) => {
    ...
    const fieldValue = field.type === 'checkbox-group'
    ...
    const stringValue = typeof fieldValue === 'string'
      ...
      ? sanitizeFormValue(fieldValue)

      : fieldValue === null || fieldValue === undefined

        ? ''

        : sanitizeFormValue(String(fieldValue));



    // For ValidatedInput components (text, email, tel, number, currency, sku)

    if (isValidatedInputType(field.type)) {
    ...
    // Add label for non-ValidatedInput fields only (ValidatedInput components have their own labels)

    const renderLabel = () => {

      // Don't render labels for ValidatedInput fields as they have their own
    ...
    switch (field.type) {
      ...
      case 'checkbox-group': {
        ...
        const renderCheckbox = (option: FormFieldOption) => {
          const safeValue = sanitizeFormValue(option.value);
          const safeLabel = sanitizeFormValue(option.label);
          const safeDescription = option.description ? sanitizeFormValue(option.description) : undefined;
          const sanitizedSelectedValues = selectedValues.map(value => sanitizeFormValue(String(value)));
          const checked = sanitizedSelectedValues.includes(safeValue);

          const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
            const updatedValues = event.target.checked
              ? Array.from(new Set([...sanitizedSelectedValues, safeValue]))
              : sanitizedSelectedValues.filter(value => value !== safeValue);

            handleFieldChange(field.name, updatedValues);
          };

          const descriptionProps = safeDescription ? { description: safeDescription } : {};
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="space-y-4">
              {field.optionGroups ? (
                field.optionGroups.map((group: FormFieldOptionGroup) => {
                  const safeGroupLabel = sanitizeFormValue(group.label);
                  const safeGroupDescription = group.description ? sanitizeFormValue(group.description) : undefined;

                  return (
                    <div
                      key={safeGroupLabel}
                      className="border border-border rounded-lg bg-surface-secondary/40 p-4"
                    >
                      <div className="mb-3">
                ...
                <div className="space-y-3">
                  {field.options.map(renderCheckbox)}
                </div>
              ) : null}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {fields.map(field => (
          ...
          <div key={field.name} className={field.type === 'textarea' ? 'md:col-span-2' : ''}>
            ...
            {/* Field Input - ValidatedInput components already include their own labels */}
