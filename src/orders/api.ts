...
export const ordersApi = {
  async list(query?: OrderQuery) {
    try {
      ...
      if ((query as any)?.customerId) params.set('customerId', (query as any).customerId);
      ...
      if (query?.includeCancelled !== undefined) params.set('includeCancelled', String(query.includeCancelled));
      if (query?.search) params.set('search', query.search);
      if (query?.limit !== undefined) params.set('limit', String(query.limit));
      const path = `/api/orders${params.toString() ? `?${params.toString()}` : ''}`;
      const res = await fetchJSON<{ orders: any[] }>(path);
      return res.orders as any[];
    } catch {
      ...
      if (query?.branchId) repoQuery.branchId = query.branchId;
      if ((query as any)?.customerId) (repoQuery as any).customerId = (query as any).customerId;
      if (query?.from !== undefined) repoQuery.from = query.from;
      if (query?.to !== undefined) repoQuery.to = query.to;
      if (query?.includeCompleted !== undefined) repoQuery.includeCompleted = query.includeCompleted;
      if (query?.includeCancelled !== undefined) repoQuery.includeCancelled = query.includeCancelled;
      return listOrders(repoQuery);
    }
  },
  ...
  async create(request: CreateOrderRequest) {
    try {
      return await postJSON('/api/orders', request);
    } catch {
      const input = {
  ...
  async updateStatus(id: string, input: UpdateOrderStatusRequest) {
    try {
      return await patchJSON(`/api/orders/${id}`, input);
    } catch {
